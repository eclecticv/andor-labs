 ...
 * This used to read `.replace(/&#\d+;/g, " ")`, turning every numeric entity
 * into a space. On any site that writes its apostrophes as `&#8217;` — which is
 * most sites built by a CMS — `We&#8217;re stepping up` came out of the crawler
 * as `We re stepping up`, and every accented character in a European company's
 * name was silently deleted.
 *
 * It stayed invisible for a long time because a language model reads "We re" as
 * "We're" without noticing, and the crawled text has never had another reader.
 * It is still wrong: this text is the evidence three jurors reason from and the
 * prose the writer quotes back, so a transformation that drops characters is
 * corrupting the input rather than cleaning it.
 */
export function toText(html: string, limit = 9_000): string {
  ...
  // left as written rather than throwing — one malformed entity on one page
  // must not cost the whole crawl.
  text = text
