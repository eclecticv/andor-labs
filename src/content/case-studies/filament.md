...
Filament helps brands and agencies improve their YouTube ROAS by eliminating 98% of low quality inventory using human-verified, daily updating exclusion lists. The WordPress site used an off-the-shelf theme that Scott Konopasek, Co-founder & COO of Filament, had to manually maintain. The design makeover, a sprint that helps startups revamp their positioning, messaging, and design system, was chosen as the right-fit.

## What we did

As part of the sprint, the founders were presented with three positioning approaches, with pros and cons for each. After a positioning was chosen, we drafted a wireframe, including homepage sections and secondary pages. We chose Next.js as the dev framework due to its extensive documentation, SSG with pre-rendered HTML, and vast component samples. For the visual centrepiece, we used a WebGL prism from the Superdesign library. A micro-animation below demonstrates how the Filament "double-sweep"—AI-based classification plus human verification—works in practice.

## The result

The core build for the website was completed in under two weeks, followed by QA passes for hardening, responsiveness, and token consistency. Out of the box, the site received excellent PageSpeed and CrUX scores during initial testing. Using Sanity as the headless CMS opens up the possibility of setting up autonomous content pipelines to optimize brand visibility across search and AI chatbots. The site codebase is hosted in a GitHub repo, which makes for easy updates via agents like Codex, Claude Code, etc.
